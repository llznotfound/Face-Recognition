...
def cv_c(train_label, train_data):
    ...
    for c in c_range:
        cmd = '-v 5 -t 0' + ' -c ' + str(c)
        cv_accuracy = svm_train(train_label, train_data, cmd)
...
for i in range(10):
    ...
    cmd = '-t 0' + ' -c ' + str(best_c)
    model = svm_train(train_label, train_data, cmd)

    t0 = datetime.now()
    t_lable, t_acc, t_val = svm_predict(test_label, test_data, model)
    t1 = datetime.now()
